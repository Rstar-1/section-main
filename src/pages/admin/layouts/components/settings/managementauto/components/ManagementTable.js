...
import AddManagement from "./add/AddManagement";
import EditManagement from "./edit/EditManagement";
...
const ManagementTable = () => {
  ...
  const getdata = useSelector(
    (state) => state.managementdata.managementdata.managementstore
  );
  const totalCount = useSelector((state) => state.managementdata.totalCount);
  // Redux State
  ...
  // API useEffect
  useEffect(() => {
    dispatch(paginationmanagement({ offset: currentpage * 6, search }));
  ...
  const handleDelete = async (id) => {
    try {
      const resultAction = await dispatch(deletemanagement(id));
  ...
  const handlePublish = async (id, newstatus) => {
    try {
      ...
      await dispatch(statusmanagement({ id, data }));
      dispatch(paginationmanagement({ offset: currentpage * 6, search }));
