...
import {
  ...
  deletedashboard,
  statusdashboard,
} from "../../../../../../../redux/automateredux/DashboardSlice";
...
const DashboardTable = () => {
  ...
  const getdata = useSelector(
    (state) => state.dashboarddata.dashboarddata.dashboardstore
  );
  const totalCount = useSelector((state) => state.dashboarddata.totalCount);
  // Redux State
  ...
  // API useEffect
  useEffect(() => {
    dispatch(paginationdashboard({ offset: currentpage * 6, search }));
  ...
  const handleDelete = async (id) => {
    try {
      const resultAction = await dispatch(deletedashboard(id));
  ...
  const handlePublish = async (id, newstatus) => {
    try {
      ...
      await dispatch(statusdashboard({ id, data }));
      dispatch(paginationdashboard({ offset: currentpage * 6, search }));
    ...
    <div className="drawer">
      ...
      >
        <div className="bgwhite wid-sidebar h-100 absolute right-0 top-0">
          ...
          <EditDashboard editshow={editshow} />
        </div>
      </div>
