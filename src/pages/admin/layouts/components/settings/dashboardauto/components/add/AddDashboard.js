...
import { adddashboard } from "../../../../../../../../redux/automateredux/DashboardSlice";
...
const AddDashboard = () => {
  ...
  const { loading, error } = useSelector((state) => state.dashboarddata);
  // Redux State
  ...
  const handleSubmit = async (e) => {
    ...
    formToSubmit.append("sectionpath", formData.sectionpath);
    formToSubmit.append("sectionname", formData.sectionname);
    formToSubmit.append("picture", formData.picture);

    const resultAction = await dispatch(adddashboard(formToSubmit));

    if (adddashboard.fulfilled.match(resultAction)) {
      setsuccess(true);
      setFormData({
        sectionpath: "",
        sectionname: "",
